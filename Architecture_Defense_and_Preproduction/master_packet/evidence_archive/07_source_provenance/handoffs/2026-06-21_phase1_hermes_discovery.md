...
**Without this env var:** defaults to `$APPDATA_ROAMING_ROOT\Hermes\` (primary profile — must not be touched by NightSchool)
...
**Effect:** Controls where the Hermes Python agent backend installs, its venv, profiles, logs, and bootstrap cache live. Defaults to `~/.hermes` (which is `$USER_HOME\.hermes\` — where the current install lives).
...
|---|---|---|
| `HERMES_DESKTOP_USER_DATA_DIR` | `$APPDATA_ROAMING_ROOT\Hermes-NightSchool\` | Electron GUI config isolation |
| `HERMES_HOME` | `$USER_HOME\.hermes-nightschool\` | Backend agent data isolation |
...
|---|---|---|
| Electron userData | `$APPDATA_ROAMING_ROOT\Hermes\` | `$APPDATA_ROAMING_ROOT\Hermes-NightSchool\` |
| Backend agent root | `$USER_HOME\.hermes\` | `$USER_HOME\.hermes-nightschool\` |
...
3. **Confirm `connection.json` is written separately** — after first NightSchool Hermes launch, verify `$APPDATA_ROAMING_ROOT\Hermes-NightSchool\connection.json` exists and primary `Hermes\connection.json` is unchanged.
