...
| 2026-06-21 | v7 | Codex | Realigned Phase 1 to the approved sequential Phase 1A shared-code lane; added Kopia Lane F guardrails; corrected the next action away from the stale fixed-port assumption | `_backups\2026-06-21_v7_phase1a-kopia-alignment.md` |
| 2026-06-21 | v8 | Codex | Added the `WorldFoundryInk` master Discord guild decision, folded the category-level permission note into the active brief, and added a parallel Hermes payload plan for guild setup plus Phase 1A review | `_backups\2026-06-21_v8_worldfoundryink-guild-alignment.md` |
...
### 4.2 Recommendation
**Go with one master Discord server, organized by category-per-project.** Current approved name for that master guild: **`WorldFoundryInk`**. NightSchool should live inside it as a dedicated category, not as a separate Discord server.

Concretely for NightSchool inside `WorldFoundryInk`:
...
- Category-level segregation is the real boundary here, not just naming. Bot role permissions do **not** override channel-level denies, so if the bot cannot post, fix the category/channel overwrites instead of giving it Administrator.
...
- `WorldFoundryInk` becomes the umbrella guild for future categories as well (`[NightSchool]`, later Codex/Claude/Hermes-adjacent projects, etc.). NightSchool is simply the first project category under that umbrella.
...
- [ ] **Discord:** create the master guild `WorldFoundryInk` if it doesn't exist yet, or confirm that `WorldFoundryInk` is the existing server where the `[NightSchool]` category will live.
...
- [ ] **Discord:** invite the bot to `WorldFoundryInk` with a role scoped to the `[NightSchool]` category — not Administrator.
...
- **Lane F — Architecture Defense (new, standing):** drift checks, persona-boundary audits, `DEPENDENCIES.md` maintenance, Kopia snapshot-boundary reviews, promotion-gate reviews. *(Claude-preferred per §3.1.)*
...
### Phase 1 — Sequential Hermes Profile Lane (Phase 1A)
*Context: reads Phase 0 status note + this section only.*

**Definition (approved 2026-06-21):** Phase 1A is a sequential alternate Hermes desktop lane, not a simultaneous second desktop instance and not a fresh NightSchool bootstrap. NightSchool reuses the existing Hermes binary and the existing Hermes agent code root, while isolating writable state through `HERMES_DESKTOP_USER_DATA_DIR` and `HERMES_HOME`. The primary Hermes desktop must be fully closed before the NightSchool lane launches.

**Stop rules:**
- Do not mutate the active primary Hermes profile at any point.
- Do not allow a NightSchool first-launch bootstrap into `.hermes-nightschool` while the installer still writes user-scoped PATH and `HERMES_HOME`.
- Do not use updater or uninstall flows from the shared-code Phase 1A lane.
...
- [ ] Card: Lane F drift check — confirm `Hermes-NightSchool\` and `.hermes-nightschool\` paths do not overlap with primary `Hermes\` or `.hermes\` paths, primary Hermes remained closed throughout launch, and no updater/uninstall action was invoked from the shared-code lane.
- [ ] Card: Lane F Kopia baseline check — trigger Policy C snapshots for primary `Hermes\` and `.hermes\` before launch; confirm the resulting evidence is recorded for the session.
...
- [ ] Card: `WorldFoundryInk` master guild + `[NightSchool]` category + channels + scoped bot role (blocked on your §6 actions).
...
Phase 0 is complete. The next work is to finish the approved Phase 1A alignment:
